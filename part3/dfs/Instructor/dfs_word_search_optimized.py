def dfs_search(maze, path, visited, word, level):
    row,column = path[-1]
    
    directions = [(-1,0),(1,0),(0,1),(0,-1)]

    if word[level] != maze[row][column]:
        return False
    ...
    for drow,dcolumn in directions:
        ...
        if (new_row >= 0 and new_row < len(maze)) and \
            ...
            if (new_row,new_column) not in visited:
                 ...
                 isFound = dfs_search(maze,path+[(new_row,new_column)],visited, word, level + 1)
...
def exist(board, word):
    ...
    for item in src_list:
        ...
        path = [start]
        level = 0
        if dfs_search(board,path,visited,word,level):
            isFound = True
            break
